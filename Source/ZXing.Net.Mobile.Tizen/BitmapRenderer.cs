...
namespace ZXing.Mobile
{
    class BitmapRenderer : IBarcodeRenderer<EvasImage>
    {
        ...
        public BitmapRenderer(Window window):base()
        {
            MainWindow = window;
        }
        ...
        public EvasImage Render(BitMatrix matrix, BarcodeFormat format, string content, EncodingOptions options)
        {
            ...
            string path = Tizen.Applications.Application.Current.DirectoryInfo.Cache + "temporary_barcode";
            Barcode​Image​Configuration barcodeImageConfiguration = new BarcodeImageConfiguration(options.Width, options.Height, path, BarcodeImageFormat.Png);
            path += ".png";

            if (type == BarcodeType.QR)
            {
                QrConfiguration qrConfig = new QrConfiguration(QrMode.Utf8, ErrorCorrectionLevel.Medium, 10);
            ...
            var evasImage = new EvasImage(MainWindow)
            {
                AlignmentX = 0.5,
                AlignmentY = 0.5,
