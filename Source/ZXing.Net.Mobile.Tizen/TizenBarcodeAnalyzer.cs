...
namespace ZXing.Mobile
{
    ...
    {
        public static async Task<Result> AnalyzeBarcodeAsync(StillImage image)
        {
            MediaVisionSource source = new MediaVisionSource(image.Data, (uint)image.Resolution.Width, (uint)image.Resolution.Height, ColorSpace.NV12);
            return await AnalyzeBarcodeAsync(source);
        }
        public static async Task<Result> AnalyzeBarcodeAsync(MediaVisionSource source)
        {
            ...
            try
            {
                ...
                if (barcodeLists.Count() == 0) return null;
                Barcode resultBarcode = barcodeLists.FirstOrDefault();
                string text = resultBarcode.Message;
                byte[] rawbytes = new Byte[source.Buffer.Length];
